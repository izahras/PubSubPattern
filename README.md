...
یا می‌توانید از Visual Studio:
- پروژه `PubSub.Example` را به عنوان Startup Project تنظیم کنید
- F5 را فشار دهید

**راه ساده‌تر: استفاده از اسکریپت‌های آماده**
...
#### ⚠️ نکته مهم: نمایش کاراکترهای فارسی در CMD

اگر کاراکترهای فارسی در CMD به صورت `?` نمایش داده می‌شوند، راه‌حل‌های زیر را امتحان کنید:

**راه‌حل 1: استفاده از اسکریپت‌های آماده** (توصیه می‌شود)
اسکریپت‌های `run-example.ps1` و `run-example.cmd` به صورت خودکار encoding را تنظیم می‌کنند.

**راه‌حل 2: استفاده از PowerShell به جای CMD**
PowerShell به صورت پیش‌فرض از UTF-8 بهتر پشتیبانی می‌کند:
```powershell
dotnet run --project PubSub.Example/PubSub.Example.csproj
```

**راه‌حل 3: تغییر Code Page در CMD**
قبل از اجرای برنامه، در CMD این دستور را اجرا کنید:
```cmd
chcp 65001
dotnet run --project PubSub.Example/PubSub.Example.csproj
```

**راه‌حل 4: استفاده از Windows Terminal**
Windows Terminal به صورت پیش‌فرض از UTF-8 پشتیبانی می‌کند و بهترین گزینه است.

**راه‌حل 5: تنظیم فونت CMD**
1. راست‌کلیک روی نوار بالای پنجره CMD
2. Properties → Font
3. فونت را به `NSimSun` یا `Consolas` تغییر دهید
...
برای اجرای تست‌ها با جزئیات بیشتر:

```bash
dotnet test --verbosity normal
```

برای اجرای تست‌های یک پروژه خاص:

```bash
dotnet test PubSub.Tests/PubSub.Tests.csproj
```
